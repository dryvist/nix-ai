...
# First Thunderbolt port with an active link; honors CLUSTER_IFACE_OVERRIDE.
cluster_detect_iface() {
  if [ -n "${CLUSTER_IFACE_OVERRIDE:-}" ]; then
    printf '%s\n' "$CLUSTER_IFACE_OVERRIDE"
    return 0
  fi
  local dev
