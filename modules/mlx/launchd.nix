  ...
  config = lib.mkIf cfg.enable {
    launchd = {
      ...
      agents = {
        ...
        # Periodic orphan reap. The launcher's reap is start-triggered, so an
        # orphan persists across an unbounded gap whenever the proxy never
        # comes back (bootout, KeepAlive off, perf-reclaim). Measured
        # 2026-07-26: a 16.7 GB orphan held a worker port 94 minutes. The
        # serving watchdog's reap_workers never loads on an mlx-lm host, so
        # this is the only reclaim path independent of a proxy start.
        # Reaps ONLY launchd-re-parented holders (ppid 1), never the live
        # worker. Rationale: scripts/mlx-orphan-reap.sh.
        mlx-orphan-reap = {
          enable = true;
          config = {
            Label = orphanReapAgentLabel;
