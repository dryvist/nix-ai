...
{
  config = lib.mkMerge [
    # Set outside the enable gate: harnesses that only *name* the skill
    # directory in an instruction file need the resolved path whether or not
    # deployment is switched on.
    { programs.agentSkills.skillsDir = skillRoot; }
