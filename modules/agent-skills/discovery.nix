...
#
# Pure SKILL.md discovery over the flake inputs handed to the agent-skills
# module. Split out of default.nix (which wires the result into the
# module's config) so each file stays under the shared-gate size limit.
{
  lib,
  pkgs,
  marketplaceInputs,
...
in
# Auto-discovers skills from every input the module receives, by trying all
# known SKILL.md layouts. No marketplace names are hardcoded here — the
# discovery walker is decoupled from Claude's registry and operates on a
# generic set of input paths supplied by the consumer flake. When this module
# is split into its own flake, the consumer decides which inputs to pass; the
# walker stays unchanged.
